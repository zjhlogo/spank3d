...
void BitmapStyleEditor::Draw(wxDC& dc)
{
	...
	for (int i = 0; i < IStyle::SS_NUM; ++i)
	{
		...
		{
			if (m_pBitmapStyle->IsAutoGenBitmap()
				&& pPieceInfo == pDefaultPieceInfo)
			{
				if (i == IStyle::SS_DOWN)
				{
					DrawPieceDarken(dc, m_rectState[i].GetPosition(), pPieceInfo, m_pBitmapStyle->IsFlipX(), m_pBitmapStyle->IsFlipY());
				}
				else if (i == IStyle::SS_HOVER)
				{
					DrawPieceHighlight(dc, m_rectState[i].GetPosition(), pPieceInfo, m_pBitmapStyle->IsFlipX(), m_pBitmapStyle->IsFlipY());
				}
				else if (i == IStyle::SS_DISABLED)
				{
					DrawPieceDisabled(dc, m_rectState[i].GetPosition(), pPieceInfo, m_pBitmapStyle->IsFlipX(), m_pBitmapStyle->IsFlipY());
				}
				else
				{
					DrawPieceNormal(dc, m_rectState[i].GetPosition(), pPieceInfo, m_pBitmapStyle->IsFlipX(), m_pBitmapStyle->IsFlipY());
				}
			}
			else
			{
